...
let owlSert = new Swiper(".owl-sert", {
  /*
	effect: 'fade',
	autoplay: {
		delay: 3000,
...
//   autoHeight: true,
  ...
  // Arrows
  navigation: {
    nextEl: ".owl__item_next",
    prevEl: ".owl__item_prev",
  },
...
function showTab(n) {
  ...
  if (n == x.length - 1) {
    document.getElementById("nextBtn").innerHTML = "Узнать результат";
  } else if (n == x.length - 2) {
    document.getElementById("nextBtn").innerHTML = "Последний вопрос";
  } else {
...
function validateForm() {
  ...
  y = x[currentTab].getElementsByClassName("test-input");
...
// ========================  Форма - заявка
(function () {
  const text = document.querySelector(".popup-pia__input--text");
  const tel = document.querySelector(".popup-pia__input--tel");
  const email = document.querySelector(".popup-pia__input--email");
  text.addEventListener("input", validation, false);
  tel.addEventListener("input", validation, false);
  email.addEventListener("input", validation, false);
  function validation() {
    if (!tel.value && !email.value) {
      tel.setCustomValidity("Введите телефон или почту");
    } else {
      tel.setCustomValidity("");
      document.querySelector(".popup_pia-popup").classList.remove("_active");
    }
  }
  validation();
})();
